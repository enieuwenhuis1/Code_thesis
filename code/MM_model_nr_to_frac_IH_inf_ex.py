...
def main():
    ...
    #     [0.0, 0.4, 0.65, 0.55],
    #     [0.3, 0.0, -0.3, -0.3],
    #     [0.6-0.278, 0.0, 0.2, 0.0],
    #     [0.55, 0.0, -0.6, 0.4]])
    #
    # df = switch_dataframe_GF_W_h(50, 1,2,7, nOC, nOB, nMMd, nMMr, growth_rates,
    #                             growth_rates_IH, decay_rates, decay_rates_IH,
    #                             matrix_no_GF_IH, matrix_GF_IH, 0.718)
    #
    ...
    #
    # print(average_MM_fraction)
    # df.plot(x = 'Generation', y = ['xOC', 'xOB', 'xMMd', 'xMMr', 'total xMM'])
    # plt.show()
    #
    #
    # """ GF H W H"""
    ...
    #     [0.0, 0.4, 0.65, 0.55],
    #     [0.3, 0.0, -0.3, -0.3],
    #     [0.6-0.033, 0.0, 0.2, 0.0],
    #     [0.55, 0.0, -0.6, 0.4]])
    #
    # df = switch_dataframe_GF_h_W_h(50, 0.858, 2.000,
    #                             5.071, nOC, nOB, nMMd, nMMr, growth_rates,
    #                             growth_rates_IH, decay_rates, decay_rates_IH,
    #                             matrix_no_GF_IH, matrix_GF_IH, 0.883)
    #
    # # Convert the number data to fraction data
    # df = number_to_fractions(df)
    #
    # # Determine the average MM fraction in the last period with and without drugs
    # time_round = 0.858 + 2.000 + 5.071 + 5.071
    # last_MM_fractions = df['total xMM'].tail(int(time_round))
    # average_MM_fraction = last_MM_fractions.sum() / (int(time_round))
    #
    # print(average_MM_fraction)
    ...
    matrix_GF_IH_comb = np.array([
        [0.0, 0.4, 0.65, 0.55],
        [0.3, 0.0, -0.3, -0.3],
        [0.6- 0.051, 0.0, 0.2, 0.0],
        [0.55, 0.0, -0.8, 0.4]])

    df = switch_dataframe_GF_comb_W_h(50, 1.910, 0.014, 5.723, 1.259,
                nOC, nOB, nMMd, nMMr, growth_rates,
                growth_rates_IH, decay_rates, decay_rates_IH, matrix_no_GF_IH,
                matrix_GF_IH, matrix_GF_IH_comb, 0.03, 0.09)

    # Convert the number data to fraction data
    df = number_to_fractions(df)

    # Determine the average MM fraction in the last period with and without drugs
    time_round = 1.910 + 0.014 + 5.723 + 1.259
    last_MM_fractions = df['total xMM'].tail(int(time_round))
    average_MM_fraction = last_MM_fractions.sum() / (int(time_round))
    ...
    df = switch_dataframe_GF_WandGF_W_h(50, 2.000, 4.995,
                4.087 , 5.919 , nOC, nOB, nMMd, nMMr, growth_rates,
                growth_rates_IH, decay_rates, decay_rates_IH, matrix_no_GF_IH,
                matrix_GF_IH, matrix_GF_IH_comb, 0.104)

    # Convert the number data to fraction data
    df = number_to_fractions(df)

    # Determine the average MM fraction in the last period with and without drugs
    time_round = 2.000 + 4.995 + 4.087  + 5.919
    last_MM_fractions = df['total xMM'].tail(int(time_round))
    average_MM_fraction = last_MM_fractions.sum() / (int(time_round))
...
def minimal_tumour_nr_to_frac_t_3_situations_IH(t_steps_IH_strength, function_order, nOC,
