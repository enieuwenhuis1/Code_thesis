...
Description:  Code with the model that simulates the dynamics in the multiple
              ...
              and OC. This was incorporated by increasing the cOC value and
              decreasing the cOB value when an IH was administered.
"""
...
def main():
    ...
    Figure_continuous_MTD_vs_AT_s_and_w_a_h(17, list_t_steps_drug)
...
def Figure_continuous_MTD_vs_AT_s_and_w_a_h(n_switches, t_steps_drug):
    ...
    # Make dataframes for continiously administration
    df_total_GF = continuous_add_IH_df(12, 150, xOC, xOB, xMMd, xMMr, N, cOC,
            cOB, cMMd, cMMr, cOC_IH, cOB_IH, matrix_no_GF_IH, matrix_GF_IH)
    df_total_WMMd = continuous_add_IH_df(12, 150, xOC, xOB, xMMd, xMMr, N, cOC,
                            cOB, cMMd, cMMr, cOC_IH, cOB_IH, matrix_no_GF_IH,
                            matrix_no_GF_IH, WMMd_inhibitor)
    df_total_comb = continuous_add_IH_df(12, 150, xOC, xOB, xMMd, xMMr, N, cOC,
                            cOB, cMMd, cMMr, cOC_IH, cOB_IH, matrix_no_GF_IH,
                            matrix_GF_IH_comb, WMMd_inhibitor_comb)

    # Print the equilibrium MMd and MMr values caused by the adaptive therapy
    last_MMd_fractions_GF = df_total_switch_GF['xMMd'].tail(int(16))
    average_MMd_fraction_GF = last_MMd_fractions_GF.sum() / 16
    last_MMr_fractions_GF = df_total_switch_GF['xMMr'].tail(int(16))
    average_MMr_fraction_GF = last_MMr_fractions_GF.sum() / 16
    print('Adaptive therapy MMd GF IH: xMMd =',average_MMd_fraction_GF,
                                        'and xMMr =', average_MMr_fraction_GF)

    last_MMd_fractions_WMMd = df_total_switch_WMMd['xMMd'].tail(int(16))
    average_MMd_fraction_WMMd = last_MMd_fractions_WMMd.sum() / 16
    last_MMr_fractions_WMMd = df_total_switch_WMMd['xMMr'].tail(int(16))
    average_MMr_fraction_WMMd = last_MMr_fractions_WMMd.sum() / 16
    print('Adaptive therapy WMMd IH: xMMd =',average_MMd_fraction_WMMd,
                                        'and xMMr =', average_MMr_fraction_WMMd)

    last_MMd_fractions_comb = df_total_switch_comb['xMMd'].tail(int(16))
    average_MMd_fraction_comb = last_MMd_fractions_comb.sum() / 16
    last_MMr_fractions_comb = df_total_switch_comb['xMMr'].tail(int(16))
    average_MMr_fraction_comb = last_MMr_fractions_comb.sum() / 16
    ...
    last_MMd_fractions_GF = df_total_GF['xMMd'].tail(int(16))
    average_MMd_fraction_GF = last_MMd_fractions_GF.sum() / 16
    last_MMr_fractions_GF = df_total_GF['xMMr'].tail(int(16))
    average_MMr_fraction_GF = last_MMr_fractions_GF.sum() / 16
    print(' Traditional therapy MMd GF IH: xMMd =',average_MMd_fraction_GF,
                                        'and xMMr =', average_MMr_fraction_GF)

    last_MMd_fractions_WMMd = df_total_WMMd['xMMd'].tail(int(16))
    average_MMd_fraction_WMMd = last_MMd_fractions_WMMd.sum() / 16
    last_MMr_fractions_WMMd = df_total_WMMd['xMMr'].tail(int(16))
    average_MMr_fraction_WMMd = last_MMr_fractions_WMMd.sum() / 16
    print(' Traditional therapy WMMd IH: xMMd =',average_MMd_fraction_WMMd,
                                        'and xMMr =', average_MMr_fraction_WMMd)

    last_MMd_fractions_comb = df_total_comb['xMMd'].tail(int(16))
    average_MMd_fraction_comb = last_MMd_fractions_comb.sum() / 16
    last_MMr_fractions_comb = df_total_comb['xMMr'].tail(int(16))
    average_MMr_fraction_comb = last_MMr_fractions_comb.sum() / 16
