...
def main():
    ...
    def model(y, t):
        if y > 16:
            return 0  # Return 0 to halt the integration
        return y + 10
    ...
    # by adaptive therapy
    list_t_steps_drug = [5, 5, 5]
    Figure_continuous_MTD_vs_AT_realistic(64, list_t_steps_drug)
    #
...
def model_dynamics_GF(y, t):
    ...
    [744654.2266544278, 1489.0458359418838, 6825.972291449797, 270.98955659630434]
    """
    nOC, nOB, nMMd, nMMr, x = y
    print('x', x)

    if x == 0:
        x_change = int(0)
        growth_rates = [0.7, 1.3, 0.3, 0.3]
        decay_rates= [1.0, 0.08, 0.2, 0.1]
    ...
    return [nOC_change, nOB_change, nMMd_change, nMMr_change, x_change]


def model_dynamics_W(y, t):
    """Function that determines the number dynamics in a population over time.
    ...
    if x == 0:
        ...
        # Payoff matrix when no GF IH drugs are present
        matrix = np.array([
            [0.0, 0.4, 0.6, 0.55],
            [0.3, 0.0, -0.3, -0.3],
            [0.6, 0.0, 0.5, 0.0],
...
def continuous_add_IH_df_GF(time_IH, end_generation, nOC, nOB, nMMd, nMMr,
    ...
    t = np.linspace(time_IH, end_generation, end_generation-time_IH)
    y0 = [nOC, nOB, nMMd, nMMr, int(0)]

    # Determine the ODE solutions
    y = odeint(model_dynamics_GF, y0, t)
...
def continuous_add_IH_df_W(time_IH, end_generation, nOC, nOB, nMMd, nMMr,
    growth_rates, growth_rates_IH, decay_rates, decay_rates_IH, matrix_no_GF_IH):
    """ Function that makes a dataframe of the cell type numbers when the IHs
    are administered continuously.
    ...
    # Determine the ODE solutions
    y = odeint(model_dynamics_W, y0, t)
    df_2 = pd.DataFrame({'Generation': t, 'nOC': y[:, 0], 'nOB': y[:, 1],
                'nMMd': y[:, 2], 'nMMr': y[:, 3], 'total nMM': y[:, 3]+ y[:, 2]})
...
def continuous_add_IH_df_comb(time_IH, end_generation, nOC, nOB, nMMd, nMMr,
    growth_rates, growth_rates_IH, decay_rates, decay_rates_IH, matrix_no_GF_IH):
    """ Function that makes a dataframe of the cell type numbers when the IHs
    are administered continuously.
    ...
    # Determine the ODE solutions
    y = odeint(model_dynamics_comb, y0, t)
    df_2 = pd.DataFrame({'Generation': t, 'nOC': y[:, 0], 'nOB': y[:, 1],
                'nMMd': y[:, 2], 'nMMr': y[:, 3], 'total nMM': y[:, 3]+ y[:, 2]})
...
def continuous_add_IH_df(time_IH, end_generation, nOC, nOB, nMMd, nMMr,
    ...
    # Determine the ODE solutions
    y = odeint(model_dynamics_golf, y0, t, args=parameters)
    df_2 = pd.DataFrame({'Generation': t, 'nOC': y[:, 0], 'nOB': y[:, 1],
                'nMMd': y[:, 2], 'nMMr': y[:, 3], 'total nMM': y[:, 3]+ y[:, 2]})
...
def Figure_continuous_MTD_vs_AT_realistic(n_switches, t_steps_drug):
    ...
    # Make dataframes for continiously administration
    df_total_GF = continuous_add_IH_df_GF(30, 350, nOC, nOB, nMMd, nMMr,
                            growth_rates, growth_rates_IH, decay_rates,
                            decay_rates_IH, matrix_no_GF_IH)
    df_total_WMMd = continuous_add_IH_df_W(30, 350, nOC, nOB, nMMd, nMMr,
                    growth_rates, growth_rates_IH, decay_rates, decay_rates_IH,
                    matrix_no_GF_IH)
    df_total_comb = continuous_add_IH_df_comb(30, 350, nOC, nOB, nMMd, nMMr,
                    growth_rates, growth_rates_IH, decay_rates, decay_rates_IH,
                    matrix_no_GF_IH)
